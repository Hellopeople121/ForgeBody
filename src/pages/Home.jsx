...
function saveAll(plan, profile) {
  try {
    localStorage.setItem(
      PLAN_KEY,
      JSON.stringify({ plan, profile, savedAt: new Date().toISOString() }),
    );
...
function buildWorkoutPrompt(form) {
  ...
  for (let i = 0; i < 7 && workoutDayIndexes.length < days; i++) {
    if (
      ...
      i >= workoutDayIndexes[workoutDayIndexes.length - 1] + gap
    ) {
...
function buildSupplementsPrompt(form) {
  return `List 5 supplements for goal="${form.goal}", fitness level="${form.fitnessLevel}". Return ONLY JSON, no other text.
...
function validateAndFix(workout, diet, supplements, peds, form) {
  ...
  while (wp.length < 7) {
    wp.push({
      day: dayNames[wp.length],
      is_rest: true,
      exercises: [],
      warmup: [],
      calories_burned: 0,
      recovery_tip: "Rest and recover.",
    });
  }
  wp = wp.slice(0, 7);
  ...
  wp = wp.map((d, i) => {
    ...
    return {
      ...
      recovery_tip: isRest
        ? d.recovery_tip || "Rest, stretch, and hydrate."
        : null,
    };
  });
...
async function generatePlan(form) {
  ...
  const workoutPromise = callGroq([
    {
      role: "system",
      content:
        "You are a fitness coach. Return ONLY valid JSON. No markdown, no explanation.",
    },
    { role: "user", content: buildWorkoutPrompt(form) },
  ]);

  const dietPromise = form.includeDiet
    ? callGroq([
        {
          role: "system",
          content:
  ...
  const suppPromise = form.includeSupplements
    ? callGroq([
        {
          role: "system",
          content:
            "You are a fitness supplement expert. Return ONLY valid JSON. No markdown, no explanation.",
        },
        { role: "user", content: buildSupplementsPrompt(form) },
      ])
    : Promise.resolve(null);

  const pedPromise = form.includeSteroids
    ? callGroq([
        {
          role: "system",
          content:
            "You are a pharmacology expert. Return ONLY valid JSON. No markdown, no explanation.",
        },
    ...
    >
      <div style={{ maxWidth: "680px", margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
          ...
          <p
            style={{ color: "#94a3b8", marginTop: "0.5rem", fontSize: "1rem" }}
          >
          ...
          <div style={{ display: "grid", gap: "1.1rem" }}>
            ...
            >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => f("fitnessLevel", e.target.value)}
                  style={sS}
                >
            ...
            <div>
              ...
              >
                <select
                  ...
                  onChange={(e) => f("heightUnit", e.target.value)}
                  style={{ ...sS, width: "auto", flexShrink: 0 }}
                >
                ...
                {form.heightUnit === "cm" ? (
                  <input
                    ...
                    placeholder="e.g. 175"
                    value={form.heightCm}
                    onChange={(e) => f("heightCm", e.target.value)}
                    style={{ ...iS, flex: 1 }}
                  />
                ) : (
                  <>
                    <input
                      ...
                      max="8"
                      value={form.heightFt}
                      onChange={(e) => f("heightFt", e.target.value)}
                      style={{ ...iS, flex: 1, minWidth: "70px" }}
                    />
                    <input
                      type="number"
                      placeholder="in"
                      ...
                      max="11"
                      value={form.heightIn}
                      onChange={(e) => f("heightIn", e.target.value)}
                      style={{ ...iS, flex: 1, minWidth: "70px" }}
                    />
                  </>
                )}
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                <input
                  ...
                  onChange={(e) => f("bodyWeight", e.target.value)}
                  style={{ ...iS, flex: 1 }}
                />
                <select
                  value={form.weightUnit}
                  onChange={(e) => f("weightUnit", e.target.value)}
                  style={{ ...sS, width: "auto" }}
                >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => f("daysPerWeek", e.target.value)}
                style={sS}
              >
                {["1", "2", "3", "4", "5", "6", "7"].map((d) => (
            ...
            {age >= 20 && (
              ...
              >
                ...
                >
                  <button
                    onClick={() => f("includeSteroids", !form.includeSteroids)}
                    style={{
                      flexShrink: 0,
                      marginTop: "2px",
                      width: "20px",
                      height: "20px",
                      borderRadius: "4px",
                      border: `2px solid ${form.includeSteroids ? "#ef4444" : "rgba(239,68,68,0.4)"}`,
                  ...
                  <div>
                    ...
                    >
                      For educational purposes only. Consult a doctor before
                      use.
                    </p>
                  </div>
                </div>
            ...
            <div>
              ...
              <textarea
                placeholder="e.g. bad knee, no gym equipment, vegetarian..."
                value={form.notes}
